...
class UnpairedImagePaths(Dataset):
    ...
    def __getitem__(self, i):
        ...
        # train
        idx1 = i
        idx2 = random.randrange(len(self.paths2))
        # idx1 = i // len(self.paths2) % len(self.paths1)  # Get the index for paths1
        # idx2 = i % len(self.paths2)  # Get the index for paths2

        # test
        # idx1 = i // len(self.paths2) % len(self.paths1)  # Get the index for paths1
        # idx2 = i % len(self.paths2)  # Get the index for paths2

        example["image1"], example["image1_path"] = self.preprocess_image(self.paths1[idx1])


        example["image2"], example["image2_path"] = self.preprocess_image(self.paths2[idx2])
        if self.numpy_folder1 is not None:
            example["numpy1"] = np.load(self.numpy_paths1[idx1])
        if self.numpy_folder2 is not None:
            example["numpy2"] = np.load(self.numpy_paths2[idx2])
        return example
